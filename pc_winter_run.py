...
def parse_args():
    ...
    parser.add_argument('--dataset', default='WikiCS', help='Input dataset.')
...
# 
# Namespace(dataset='WikiCS', num_hops=2, seed=101, num_perm=10, label_trunc_ratio=0, group_trunc_ratio_hop_1=0.5, group_trunc_ratio_hop_2=0.7, verbose=True)
args = parse_args()
print(args)
...
for i in range(num_perm):
    ...
    loopcount = len(labeled_node_list[:trunc_label_len])
    print(f"loop-->{loopcount}")
    
    for labeled_node in labeled_node_list[:trunc_label_len]:
        print(f"===loop: {loopcount}============node: {labeled_node}===================================")
